...
if __name__ == "__main__":
    ...
    for i in range(num_conformations):
        ...
        write_pdb_to_file(
            receptor,
            os.path.join(destination_path, "lightdock_%s.pdb" % 0),
            receptor_pose,
        )
        write_pdb_to_file(
            ligand, os.path.join(destination_path, "lightdock_%s.pdb" % 0), ligand_pose
        )
        output_file = open(os.path.join(destination_path, "lightdock_%s.pdb" % 0), "a")
